...
/* B-SPLINES */
	...
	this.updateBSpline = function(){
		var controls = this.controls.concat();
...
/* RAGS */
	...
	function p(u,controls,sigma){
		...
		var wg = [];
		for(var i=0; i < controls.length; i++){
			wg.push(weight*g(i,u,sigma,controls.length));
		}
	...
	function g(i,u,s,n){
		return Math.exp(-Math.pow(u-(i/(n-1)),2)/(2*Math.pow(s,2)));
	}
